#!/usr/bin/env python
# -*- encoding: utf-8 -*-
#
# Copyright 2015-2016 Red Hat, Inc.
#
# Licensed under the Apache License, Version 2.0 (the "License"); you may
# not use this file except in compliance with the License. You may obtain
# a copy of the License at
#
#    http://www.apache.org/licenses/LICENSE-2.0
#
# Unless required by applicable law or agreed to in writing, software
# distributed under the License is distributed on an "AS IS" BASIS, WITHOUT
# WARRANTIES OR CONDITIONS OF ANY KIND, either express or implied. See the
# License for the specific language governing permissions and limitations
# under the License.

import dciclient.v1.api.component as dci_component
import dciclient.v1.api.context as dci_context
import dciclient.v1.api.topic as dci_topic

from datetime import datetime
from six.moves.urllib.parse import urlparse

import click
import os.path
import requests
import shutil
import StringIO
import subprocess
import tarfile

try:  # py27
    import ConfigParser as configparser
except ImportError:
    import configparser


def get_repo_information(repo_file):
    repo_file = requests.get(repo_file)
    repo_file_raw_content = repo_file.text

    repo_file_date = repo_file.headers['Last-Modified']
    dt = datetime.strptime(repo_file_date,  '%a, %d %b %Y %H:%M:%S GMT')
    repo_date = '%s-%s-%s' % (dt.year, dt.month, dt.day)

    output = StringIO.StringIO(repo_file_raw_content)
    config = configparser.ConfigParser()
    config.readfp(output)
    # we only use the first section
    section_name = config.sections()[0]
    raw_base_url = config.get(section_name, 'baseurl')
    base_url = raw_base_url.replace("$basearch", "x86_64")
    try:
        version = config.get(section_name, 'version')
    except configparser.NoOptionError:
        # extracting the version from the URL
        version = base_url.split('/')[-1]
    return base_url, version, repo_date


def get_puddle_component(repo_file, topic_id, name):
    (base_url, version, repo_date) = get_repo_information(repo_file)

    puddle_type = 'snapshot_rdo'
    puddle_component = {
        'type': puddle_type,
        'canonical_project_name': name,
        'name': '%s %s %s' % (name, repo_date, version),
        'url': base_url,
        'data': {
            'path': urlparse(base_url).path,
            'version': version,
            'repo_name': name
        },
        'topic_id': topic_id,
    }
    return puddle_component


def get_components(urls, topic_id, name):
    components = []
    for url in urls:
        components.append(get_puddle_component(url, topic_id, name))
    return components


def clean_up(component):
    project_name = component['canonical_project_name']
    if os.path.exists(project_name):
        shutil.rmtree(project_name)
    archive = 'archive.tar'
    if os.path.exists(archive):
        os.unlink(archive)
    if os.path.exists('yum.conf'):
        os.unlink('yum.conf')


def prepare_archive(component):
    project_name = component['canonical_project_name']
    config = configparser.ConfigParser()
    config.add_section(project_name)
    config.set(project_name, 'name', project_name)
    config.set(project_name, 'baseurl', component['url'])
    config.set(project_name, 'gpgcheck', 1)
    config.set(project_name, 'enabled', 1)

    with open('yum.conf', 'w') as fd:
        config.write(fd)
    subprocess.check_call([
        'reposync',
        '-c', 'yum.conf',
        '--download-metadata',
        '--norepopath',
        '-r', project_name,
        '-p', project_name])
    subprocess.check_call(['createrepo', project_name])

    archive = 'archive.tar'
    with tarfile.open(archive, 'w:') as tar:
        tar.add(project_name)


def refresh_puddles(ctx):
    versions = [
        {
            'name': 'RDO-Ocata',
            'urls': [
                 'http://buildlogs.centos.org/centos/7/cloud/x86_64/rdo-trunk-master-tested/delorean.repo',  # noqa
             ],
            'topic_id': dci_topic.get(ctx, 'RDO-Ocata').json()['topic']['id'],
        },
        {
            'name': 'RDO-Newton',
            'urls': [
                 'http://buildlogs.centos.org/centos/7/cloud/x86_64/rdo-trunk-newton-tested/delorean.repo',  # noqa
             ],
            'topic_id': dci_topic.get(ctx, 'RDO-Newton').json()['topic']['id'],
        }
    ]

    for v in versions:
        components = get_components(v['urls'], v['topic_id'], v['name'])
        for c in components:
            dci_component.create(ctx, **c)
            r = dci_component.get(ctx, c['name'], embed='files')
            if r.status_code == 401:
                continue
            if r.status_code == 404:
                print('Component %s not found' % c['name'])
                exit(1)
            component = r.json()['component']
            r_current_components = dci_component.file_list(
                ctx, id=component['id'])
            if len(r_current_components.json()['component_files']):
                print('%s already has its data. Skipped.' % component['name'])
                continue

            clean_up(component)
            prepare_archive(component)
            print('Uploading bits...')
            r = dci_component.file_upload(
                ctx, id=component['id'], file_path='archive.tar')
            if r.status_code != 201:
                print((
                    '  Upload of archive.tar has '
                    'failed for component %s: %s') % (
                        component['name'], r.text))
                exit(1)
            else:
                print('  success: %s' % r.json())
            clean_up(component)


@click.command()
@click.option('--dci-login', envvar='DCI_LOGIN', required=True,
              help="DCI username account.")
@click.option('--dci-password', envvar='DCI_PASSWORD', required=True,
              help="DCI password account.")
@click.option('--dci-cs-url', envvar='DCI_CS_URL', required=True,
              help="DCI CS url.")
def main(dci_login, dci_password, dci_cs_url):
    ctx = dci_context.build_dci_context(dci_cs_url, dci_login,
                                        dci_password)
    refresh_puddles(ctx)


if __name__ == '__main__':
    main()
